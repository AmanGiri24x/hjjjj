...
// Indian Market API proxies
app.get('/api/v1/indian-market/stocks', async (req, res) => {
  try {
    const response = await axios.get(`${PYTHON_SERVICE_URL}/api/v1/indian-market/stocks`);
    res.json(response.data);
  } catch (error) {
    res.status(500).json({ error: 'Failed to fetch Indian stocks', details: error.message });
  }
});

app.get('/api/v1/indian-market/3d-data', async (req, res) => {
  try {
    const response = await axios.get(`${PYTHON_SERVICE_URL}/api/v1/indian-market/3d-data`);
    res.json(response.data);
  } catch (error) {
    res.status(500).json({ error: 'Failed to fetch 3D data', details: error.message });
  }
});

app.get('/api/v1/indian-market/company/:symbol', async (req, res) => {
  try {
    const response = await axios.get(`${PYTHON_SERVICE_URL}/api/v1/indian-market/company/${req.params.symbol}`);
    res.json(response.data);
  } catch (error) {
...
app.get('/api/v1/indian-market/top-performers', async (req, res) => {
  try {
    const response = await axios.get(`${PYTHON_SERVICE_URL}/api/v1/indian-market/top-performers`);
    res.json(response.data);
  } catch (error) {
...
app.get('/api/v1/indian-market/sectors', async (req, res) => {
  try {
    const response = await axios.get(`${PYTHON_SERVICE_URL}/api/v1/indian-market/sectors`);
    res.json(response.data);
  } catch (error) {
    res.status(500).json({ error: 'Failed to fetch sector data', details: error.message });
  }
});
